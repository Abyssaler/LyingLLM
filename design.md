...
│   │   ├── memory/
│   │   │   ├── __init__.py
│   │   │   └── game_memory.py       # Agent游戏内记忆（三级分类）
│   │   ├── storage/
│   │   │   ├── __init__.py
│   │   │   └── game_log.py          # 对局日志持久化
│   │   └── api/
...
- **Night Phase**: 按优先级顺序执行各角色夜间技能（守卫→狼人协商并确认击杀目标→女巫→预言家）
- **Day Phase**: 通告昨夜信息 → 自由讨论 → 投票 → 处决/平票处理
- **Win Check**: 每次有人出局后检查胜负条件
...
```
WAITING ──(start)──→ SHERIFF_ELECTION ──(elected)──→ NIGHT_BEGIN
                                                        │
                    ┌───────────────────────────────────┘
                    ▼
              WOLF_DISCUSS ──(确认击杀目标)──→ NIGHT_ACTIONS ──(all done)──→ DAWN
                                                                        │
                    ┌───────────────────────────────────────────────────┘
                    ▼
              ANNOUNCE_DEATHS ──(announce)──→ LAST_WORDS ──(skip/done)──→ DISCUSS
                                                                            │
                    ┌─────────────────────────────────────────────────────┘
                    ▼
                  VOTE ──(vote done)──→ EXECUTE ──(has_last_words)──→ LAST_WORDS
                    │                                                │
                    │          ┌──────────────────────────────────────┘
                    │          ▼
                    │    ON_DEATH_SKILL ──(hunter shoots)──┐
                    │                                       │
                    ▼◄──────────────────────────────────────┘
              WIN_CHECK ──(no winner)──→ NIGHT_BEGIN
                    │
                    └───(has winner)──→ GAME_END
```
...
| 角色记忆 | 同角色/同阵营玩家 | 狼人协商记录、预言家查验结果、女巫用药记录    |
| 私有记忆 | 仅自己            | 思维链(reasoning)、个人策略、对他人角色的猜测 |
...
白天讨论阶段多 Agent 并行发言，需要对 LLM 调用做并发管理：
...
```json
{
  ...
  "timeline": [
    { "phase": "night_1", "role": "werewolf", "player": 1, 
      "action": {"type": "kill", "target": 3},
      "thinking": "3号在白天发言很激进...", 
      "timestamp": "..."},
    { "phase": "day_1", "type": "speech", "player": 2,
      "content": "我怀疑4号是狼人...",
      "thinking": "实际上我是预言家，我查了4号...",
      "timestamp": "..."},
    { "phase": "day_1", "type": "vote", "player": 2, "target": 4 }
  ],
...
5. **遗言环节**（如启用）：出局玩家发表遗言
6. **白天讨论**：由警长决定发言顺序（如警长已出局，则按默认顺序），存活玩家依次发言
...
1. **信息隔离**：Agent 的记忆系统严格按角色权限过滤（三级记忆），引擎负责分发信息
2. **异步串行执行**：夜间行动按优先级串行，但 LLM 调用是异步的；白天讨论可并行
3. **思考模式可视化**：`thinking` 字段与 `speech` 分离存储，前端可按玩家切换查看
4. **规则即Prompt**：规则文件直接转化为 Agent system prompt 的一部分，确保所有模型"读懂"规则
